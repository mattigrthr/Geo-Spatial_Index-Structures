...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    /**
     * @deprecated does not return correct result with new behavior
    ...
    private Node insert(Node h, Key key, Value val, int ht) {
         ...
         *
         * Overwrite existing key value pairs and
         * to delete key value pairs by inserting null as a value
         */

        boolean existing = false;

        /**
         * Check if key already exists in the BTree
         */
        for (int i = 0; i < h.m; i++) {
            if (h.children[i] != null && h.children[i].key.equals(key)) {
                existing = true;
        ...
        if (!existing) {
            ...
            if (val != null) {
                 ...
                 */
                for (int i = 0; i < h.m; i++) {
                    if (h.children[i].key == key) {
                        h.children[i].val = val;
                    }
                }
            } else {
                /**
                ...
                for (int i = 0; i < h.m; i++) {
                    if (h.children[i].key == key) {
                        for (int k = i; j < h.children.length - 1; j++){
                            h.children[k] = h.children[k+1];
                        }

                        if (h.children[h.children.length - 1] != null) {
                            h.children[h.children.length - 1] = null;
                        }
    ...
    public static void main(String[] args) {
        ...
        st.put("www.cs.princeton.edu", null);
